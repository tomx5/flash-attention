...
@pytest.mark.parametrize("rotary_use_fused", [True])
# @pytest.mark.parametrize('seqlen_q,seqlen_k', [(256, 128)])
def test_flash_attn_kvcache(
    ...
    out = flash_attn_with_kvcache(
        q if rotary_use_fused else q_ro,
        k_cache if paged_kv_block_size is None else k_cache_paged,
        v_cache if paged_kv_block_size is None else v_cache_paged,
        k if rotary_use_fused else k_ro,
        v,
        rotary_cos=cos if rotary_use_fused else None,
        rotary_sin=sin if rotary_use_fused else None,
        rotary_cos_k=None,
        rotary_sin_k=None,
        rotary_dim=rotary_dim if rotary_use_fused else 0,
