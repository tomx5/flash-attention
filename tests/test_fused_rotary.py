...
def rotary_kernel_splitk(
    ...
    start_m,        # the token idx the current M_BLOCK starts at.
    ...
    if not INTERLEAVED:
        ...
        x0_range = range_m[:, None]*stride_m + range_d_repeat[None, :]                # BLOCK_M x 1st half of headdim (fast to load)
        x1_range = range_m[:, None]*stride_m + range_d_repeat[None, :] + ro_dim_half  # BLOCK_M x 2nd half of headdim (fast to load)
    else:
...
def split(
    ...
    cs_range = (SEQLEN_OFFSET + tl.arange(0, BLOCK_M))[:, None] + tl.arange(0, rotary_dim//2)
